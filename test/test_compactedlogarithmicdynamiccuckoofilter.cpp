...
#include "../benchmark/impl/compactedlogarithmicdynamiccuckoofilter/compactedlogarithmicdynamiccuckoofilter.hpp"

using compactedlogarithmicdynamiccuckoofilter::CompactedLogarithmicDynamicCuckooFilter;
...
TEST_CASE(
    "CompactedLogarithmicDynamicCuckooFilter should perform insertions/query/deletion correctly",
  ...
  SECTION("Deletion should work correctly") {
    ...
    // Should have very low false positive since we deleted all the inserted elements
    size_t false_positive = 0;
    for (size_t i = 0; i < INSERT_NUM; i++)
      if (filter.queryItem(nums[i]))
        false_positive++;
    REQUIRE(false_positive < INSERT_NUM * 0.01);
