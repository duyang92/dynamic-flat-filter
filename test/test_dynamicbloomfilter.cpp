...
#include "../benchmark/impl/dynamicbloomfilter/dynamicbloomfilter.hpp"
#include "../benchmark/predefine.hpp"

using dynamicbloomfilter::DynamicBloomFilter;
...
TEST_CASE("DynamicBloomFilter should perform insertions/query/deletion correctly",
          "[dynamicbloomfilter]") {
  ...
  SECTION("Deletion should work correctly") {
    // As mentioned in the paper of Dynamic Cuckoo Filter (Hanhua Chen, Liangyi Liao, Hai Jin, Jie
    // Wu, "The Dynamic Cuckoo Filter," Proceedings of the 25th IEEE International Conference on
    // Network Protocols (ICNP 2017), Toronto, Canada, Oct. 10-13, 2017.), it does not support
    // reliable deletion, so we only expect a relatively high deletion rate.

    size_t delete_success = 0;
    for (size_t i = 0; i < INSERT_NUM; i++)
      if (filter.deleteItem(nums[i]))
        delete_success++;
    REQUIRE(static_cast<double>(delete_success) / static_cast<double>(INSERT_NUM) > 0.9);

    // Should have low false positive since we deleted mostinserted elements
    size_t false_positive = 0;
    for (size_t i = 0; i < INSERT_NUM; i++)
